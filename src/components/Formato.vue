        ...
        <md-field>
          ...
          <md-select v-model="destinatario" v-bind:style="opcion" name="destinatario">
            <md-option
              ...
              v-bind:style="opcion"
              v-bind:value="destinatario.nombre"
            >{{destinatario.nombre}}</md-option>
          </md-select>
        </md-field>
        <md-field>
          <label>Asunto</label>
          <md-textarea v-model="asunto" md-autogrow></md-textarea>
        </md-field>
        <md-field>
...
export default {
  ...
  methods: {
    ...
    encriptar: function() {
      ...
      encryptionKey = CryptoJS.enc.Hex.parse(
        this.convertBytesToHex(encryptionKey)
      );
      this.llave_cifrado = RSA.KJUR.crypto.Cipher.encrypt(
        encryptionKey.toString(),
        RSA.KEYUTIL.getKey(this.destinatario_public)
      );
      this.contenido_cifrado = CryptoJS.AES.encrypt(
        JSON.stringify(this.email.slice(-1)[0]),
        encryptionKey.toString()
      ).toString();
    },
    setDestinatario() {
        ...
        db.collection("usuarios")
          ...
          .then(doc => {
            this.mensaje_verificacion =
              "Mensaje cifrado enviado con id : " + " " + doc.id;
          });
      } else {
        db.collection("usuarios")
          .doc(this.destinatario_rfc)
          .collection("emails")
          .add(this.email.slice(-1)[0])
          .then(doc => {
            this.mensaje_verificacion =
              "Mensaje enviado con id  " + " " + doc.id;
          });
      }
      this.dialog.show();
      db.collection("usuarios")
        .doc(this.rfc)
        .collection("bandejaSalida")
        .add(this.email.slice(-1)[0])
        .then(doc => {
          alert(doc.id);
        });
